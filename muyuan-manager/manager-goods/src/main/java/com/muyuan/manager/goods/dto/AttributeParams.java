...
public class AttributeParams {
    ...
    @NotNull(message = "categoryCode 分类Id不能为空",groups = {Add.class,Update.class})
    /** 商品分类ID */
    @ApiModelProperty(value = "商品分类Code")
    private Long categoryCode;

    @NotNull(message = "属性类型不能为空",groups = {Add.class,Update.class})
    /** 属性类型 1:关键属性 2:销售属性 3:非关键属性 */
    @ApiModelProperty(value = " 属性类型 转换为二进制 1:公共属性 10:销售属性 100:关键属性 1000:非关键属性 type值为类型的和")
    private Integer type;

    @NotNull(message = "取值类型不能为空",groups = {Add.class,Update.class})
